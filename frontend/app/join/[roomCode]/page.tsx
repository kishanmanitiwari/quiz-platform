  ...
  async function join() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (errString.includes("unique") || errString.includes("already") || errString.includes("p2002")) {
          return setMessage("You have already joined or played this quiz with this WhatsApp number!");
        }
    ...
    return (
      <main className="min-h-screen bg-slate-50 px-4 py-8">
        <section className="mx-auto max-w-md rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-8">
            <label className="block text-sm font-semibold text-slate-800">Your Name</label>
            <input 
          ...
          <div className="mt-5">
            <label className="block text-sm font-semibold text-slate-800">WhatsApp Number</label>
            <input 
              type="tel"
              className="focus-ring mt-2 w-full rounded-md border border-slate-300 bg-white px-4 py-3" 
              value={phone} 
            ...
            <p className="mt-2 text-xs font-medium text-slate-500">
              * Results, gift coupons, and photos for today's event will be shared on this number.
            </p>
          </div>
        ...
        <section className="mx-auto max-w-xl">
          ...
          <div className="mt-6 grid gap-3">
            {options.map(([key, label]) => (
              <button key={key} disabled={state.alreadyAnswered} onClick={() => answer(key)} className="focus-ring rounded-md border border-slate-300 bg-white p-4 text-left font-semibold transition-colors hover:bg-slate-50 disabled:opacity-60">
                <span className="mr-2 text-leaf">{key}.</span>{label}
              </button>
            ))}
          </div>
          {state.alreadyAnswered && <p className="mt-5 rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-900">Answer submitted. Waiting for next question...</p>}
