...
app.post("/api/admin/rooms", requireAdmin, asyncHandler(async (req, res) => {
  ...
  if (questionCount !== 6) return res.status(400).json({ error: "Quiz must have exactly 6 questions" });
...
app.post("/api/join", asyncHandler(async (req, res) => {
  ...
  if (room.status !== "WAITING") {
    return res.status(409).json({ error: "Quiz has already started. Please join the next session." });
  }

  const participant = await prisma.participant.upsert({
    where: { roomId_sessionId: { roomId: room.id, sessionId: parsed.data.sessionId } },
    update: { name: cleanName(parsed.data.name) },
    create: { roomId: room.id, sessionId: parsed.data.sessionId, name: cleanName(parsed.data.name) }
  });
