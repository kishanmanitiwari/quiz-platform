...
export function createSocketServer(server: Server) {
  ...
  io.on("connection", (socket) => {
    const attachParticipant = async (
      ...
      try {
        ...
        if (!participant)
          return callback?.({
            ok: false,
            error: "Participant not found for this room",
          });
        socket.data.participantId = participant.id;
        socket.data.roomId = participant.roomId;
        socket.join(socketRoom(participant.room.roomCode));
        const state = await getPublicRoomState(
          prisma,
          participant.roomId,
          participant.id,
        );
        callback?.({ ok: true, state });
        io.to(socketRoom(participant.room.roomCode)).emit(
          "participant:joined",
          {
    ...
    socket.on("admin:join", async ({ roomId }, callback) => {
      ...
      callback?.({
        ok: true,
        state: await getPublicRoomState(prisma, room.id),
      });
    });
    ...
    socket.on("question:start", async ({ roomId }, callback) => {
      ...
      if (!["ACTIVE", "BETWEEN_QUESTIONS"].includes(room.status)) {
        return callback?.({
          ok: false,
          error: "Cannot start next question now",
        });
      }
      const nextQuestion = room.quiz.questions.find(
        (q) => q.order === room.currentQuestion + 1,
      );
      if (!nextQuestion)
    ...
    socket.on("answer:submit", async (payload, callback) => {
      try {
        ...
        if (now.getTime() > room.questionEndsAt.getTime()) {
          return callback?.({
            ok: false,
            error: "Answer submitted after deadline",
          });
        }
        const question = await prisma.question.findFirst({
          where: {
            ...
            quizId: room.quizId,
            order: room.currentQuestion,
          },
        });
        if (!question)
          return callback?.({
            ok: false,
            error: "Invalid question for this room",
          });
        ...
        callback?.({
          ok: true,
          answer: { id: answer.id, score: answer.score },
        });
        ...
        io.to(socketRoom(participant.room.roomCode)).emit(
          "leaderboard:update",
          await getLeaderboard(prisma, participant.roomId),
        );
      } catch (error: any) {
        if (error?.code === "P2002")
          return callback?.({ ok: false, error: "Answer already submitted" });
    ...
    socket.on("question:end", async ({ roomId }, callback) => {
      ...
      // Prevent duplicate question:end requests
      if (room.status !== "QUESTION_ACTIVE") {
        return callback?.({
          ok: false,
          error: "No active question to end",
        });
      }
      ...
      const updated = await prisma.room.update({
        where: { id: room.id },
        data: {
          status: isLast ? "FINISHED" : "BETWEEN_QUESTIONS",
          questionEndsAt: new Date(),
        },
      });

      const leaderboard = isLast
        ? await persistResults(prisma, room.id)
        : await getLeaderboard(prisma, room.id);

      io.to(socketRoom(room.roomCode)).emit(
        isLast ? "quiz:finish" : "question:end",
        {
          room: updated,
          leaderboard,
        },
    ...
    socket.on("quiz:finish", async ({ roomId }, callback) => {
      ...
      io.to(socketRoom(room.roomCode)).emit("quiz:finish", {
        room: updated,
        leaderboard,
      });
